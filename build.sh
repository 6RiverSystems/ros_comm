...
COMMAND="fpm -x 'opt/mfp_chuck/.*' -x 'opt/mfp_chuck/_setup*' -x 'opt/mfp_chuck/local_setup*' -x 'opt/mfp_chuck/env.sh' -x 'opt/mfp_chuck/setup*' -s dir -t deb -n ros-comm --version ${VERSION} install/=/opt/mfp_chuck"
echo "${COMMAND}"
eval "${COMMAND}"
cp *.deb ${WORKSPACE}/
